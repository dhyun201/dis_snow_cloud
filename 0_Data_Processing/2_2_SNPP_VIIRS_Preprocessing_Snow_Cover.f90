...
!      ELSE IF (viirs_data(i,j) == 4) THEN
!        geo_index_x_viirs(i,j) = NINT( (lon_viirs(i,j)-stlon)/pixel_size )+1
!        geo_index_y_viirs(i,j) = NINT( (stlat-lat_viirs(i,j))/pixel_size )+1
!        geo_viirs_snow(geo_index_x_viirs(i,j),geo_index_y_viirs(i,j)) = 4  !! VIIRS Cirrus
      ENDIF

      ENDDO
      !IF ( MOD(j, 1000) == 0 ) PRINT*, j, ' / ', vy, ', month : ', month_loop(kk)
   ENDDO

   !OPEN(99,FILE=TRIM(output_path)//'VIIRS_Snow_AMI_Map_'//Trim(date(1:6))//&
   !        '/VIIRS_Geographic_Map_2km_'//date//time//'.bin', &
   !        ACCESS='DIRECT',RECL=wx*wy*1)
   !WRITE(99,REC=1) geo_viirs_snow
   !CLOSE(99)
...
!         ELSE IF (geo_viirs_snow(geo_index_x_ami(i,j),geo_index_y_ami(i,j)) == 4) THEN
!            snow_ami(i,j) = 4  !! Cirrus
         ENDIF         
