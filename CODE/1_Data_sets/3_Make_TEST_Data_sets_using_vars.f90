...
INTEGER(KIND=2)                                 :: ct
INTEGER(KIND=4)                                 :: low_conf_cld_ct, gk2a_cld_ct
...
INTEGER(KIND=2),DIMENSION(gx,gy)                :: gk2a_ch01_2km_dn, &
                                                   ...
                                                   gk2a_landcover_2km
!!==============================================================================
INTEGER(KIND=2),DIMENSION(gx,gy)                :: gk2a_sol_zen_2km_int, &
                                                   gk2a_sat_zen_2km_int, &
                                                   btd_14_07_int, &
                                                   gk2a_ch06_2km_ref_int, &
                                                   gk2a_ch05_2km_ref_int, &
                                                   gk2a_ch07_2km_bt_int, &
                                                   gk2a_ch15_2km_bt_int, &
                                                   gk2a_ch14_2km_bt_int, &
                                                   ref_ano_dim_int, &
                                                   ndsi_int, &
                                                   ndwi_int, &
                                                   gk2a_lat_2km_int, &
...
REAL(KIND=4),DIMENSION(23)                      :: out_flag
...
opath_gk2a_dataset   = '/data/DHJIN/PhD/Data_TrainTest/Data_Test.Scene_v3_All/'
...
PRINT*, ' [ Reading Complete : GK-2A Lat./Lon./lsmask/DEM/Lancover/VZA ] '
...
!!-----------------------------------------------------------------
OPEN(11,FILE=TRIM(ipath_viirs_filelist_train)//'date_sc_filelist_train.test_TEST_dataset.txt',&
        STATUS='OLD')
ct = 0
DO WHILE(.TRUE.)
...
DO kk = 1, ct
  ...
  !if (sc_ami_date(kk) == '202001270200') then
  !  print*, "Oh!!"
  !else if (sc_ami_date(kk) /= '202001270200') then
  !  cycle
  !endif
  ...
  ipath_gk2a_l1b = '/data/DHJIN/DATA_GK2A.AMI/L1B/1_BIN.File/'//&
    ...
    IF ( istat /= 0 ) THEN
      PRINT*, "NO FILE : VIIRS SNOW COVER"
      PRINT*, TRIM(sc_viirs_date(kk))
      CYCLE
    ENDIF
    ...
    OPEN(21,FILE=TRIM(ipath_gk2a_cld)//'gk2a_ami_le2_cld_fd020ge_'//&
            ...
            STATUS='OLD', ACCESS='DIRECT', RECL=gx*gy*1,IOSTAT=istat)
    IF ( istat /= 0 ) THEN
      PRINT*, "NO FILE : GK-2A CLOUD MASK"
      CYCLE
    ENDIF
...
!!-----------------------------------------------------------------------
  ...
  flag_info = -99
  gk2a_ind_x_int = 0  ;  gk2a_ind_y_int = 0
  viirs_data_int = -99
...
!!-----------------------------------------------------------------------
        ...
        !! Night Check (SZA > 80)
        IF ( gk2a_sol_zen_2km(i,j) > 80.0 ) THEN
          flag_info(i,j) = 3
          CYCLE
        ENDIF

        !! Check fill-value & sea
        IF (gk2a_lsmask_2km(i,j) == 0) THEN
          flag_info(i,j) = 1
          CYCLE
        ENDIF

        !! Check viirs data
!        IF ( viirs_data(i,j) == -128 .OR. &
!             viirs_data(i,j) ==  -10 .OR. &
!             viirs_data(i,j) ==   10 ) THEN
!          flag_info(i,j) = 2
!          CYCLE
!        ENDIF

        !! High Clear Flag Check (GK-2A CLD)
        IF ( gk2a_cld_2km(i,j) == 2 ) THEN
          flag_info(i,j) = 4
          CYCLE
        ENDIF

        !! High Cloudy Flag Check (GK-2A CLD)
        IF ( gk2a_cld_2km(i,j) == 0 ) THEN
          flag_info(i,j) = 5
         CYCLE
        ENDIF

        !! Check Background Snow Cover
        IF (gk2a_background_sc_2km(i,j) /= 1) THEN
          flag_info(i,j) = 2
          CYCLE
        ENDIF

        !! High Cloudy Flag Check (GK-2A CLD)
        IF ( gk2a_cld_2km(i,j) == 1 ) THEN
!        IF ( gk2a_cld_2km(i,j) == 1 .OR. gk2a_cld_2km(i,j) == 0 ) THEN
          flag_info(i,j) = 6
          low_conf_cld_ct = low_conf_cld_ct + 1
!          gk2a_cld_ct = gk2a_cld_ct + 1
        ENDIF

!!-----------------------------------------------------------------------
!!-----------------------------------------------------------------------
        !! Input Index X, Y variable
        gk2a_ind_x_int(i,j) = i
        gk2a_ind_y_int(i,j) = j
        viirs_data_int(i,j) = viirs_data(i,j)
!!-----------------------------------------------------------------------
!!-----------------------------------------------------------------------

        !! Min check
        IF ( gk2a_ch01_2km_ref(i,j) < 0.01 ) gk2a_ch01_2km_ref(i,j) = 0.01                                          
        IF ( gk2a_ch02_2km_ref(i,j) < 0.01 ) gk2a_ch02_2km_ref(i,j) = 0.01                                          
        IF ( gk2a_ch03_2km_ref(i,j) < 0.01 ) gk2a_ch03_2km_ref(i,j) = 0.01                                          
        IF ( gk2a_ch04_2km_ref(i,j) < 0.01 ) gk2a_ch04_2km_ref(i,j) = 0.01                                          
        IF ( gk2a_ch05_2km_ref(i,j) < 0.01 ) gk2a_ch05_2km_ref(i,j) = 0.01                                          
        IF ( gk2a_ch06_2km_ref(i,j) < 0.01 ) gk2a_ch06_2km_ref(i,j) = 0.01                                          
        ...
        ref_ano_dim(i,j) = ref_ano

!!-----------------------------------------------------------------------
        !! Check the variable range
!        IF ( ref_ano_dim(i,j) < -2.0 ) ref_ano_dim(i,j) = -2.0
!        IF ( ref_ano_dim(i,j) >  2.0 ) ref_ano_dim(i,j) =  2.0

!        IF ( ndwi(i,j) < -0.5 ) ndwi(i,j) = -0.5
!        IF ( ndwi(i,j) >  1.0 ) ndwi(i,j) =  1.0
       
!        IF ( ndsi(i,j) < -1.0 ) ndwi(i,j) = -1.0
!        IF ( ndsi(i,j) >  1.0 ) ndwi(i,j) =  1.0
        
!        IF ( gk2a_ch07_2km_bt(i,j) < 240.0) gk2a_ch07_2km_bt(i,j) = 240.0
!        IF ( gk2a_ch07_2km_bt(i,j) > 380.0) gk2a_ch07_2km_bt(i,j) = 380.0

!        IF ( gk2a_ch14_2km_bt(i,j) < 210.0) gk2a_ch14_2km_bt(i,j) = 210.0
!        IF ( gk2a_ch14_2km_bt(i,j) > 320.0) gk2a_ch14_2km_bt(i,j) = 320.0
        
!        IF ( gk2a_ch15_2km_bt(i,j) < 210.0) gk2a_ch15_2km_bt(i,j) = 210.0
!        IF ( gk2a_ch15_2km_bt(i,j) > 310.0) gk2a_ch15_2km_bt(i,j) = 310.0
 
!        IF ( gk2a_lat_2km(i,j) < 20.0 ) gk2a_lat_2km(i,j) = 20.0
!        IF ( gk2a_lat_2km(i,j) > 80.0 ) gk2a_lat_2km(i,j) = 80.0

!        IF ( gk2a_dem_2km(i,j) < 0.1   ) gk2a_dem_2km(i,j) =    0.1
!        IF ( gk2a_dem_2km(i,j) > 8000.0) gk2a_dem_2km(i,j) = 8000.0

!        IF ( gk2a_sat_zen_2km(i,j) < 30.0 ) gk2a_sat_zen_2km(i,j) = 30.0
!        IF ( gk2a_sat_zen_2km(i,j) > 90.0 ) gk2a_sat_zen_2km(i,j) = 90.0

!!-----------------------------------------------------------------------
        !! Input the value for the variable
!        gk2a_sol_zen_2km_int(i,j)  = gk2a_sol_zen_2km(i,j) * 100
!        gk2a_sat_zen_2km_int(i,j)  = gk2a_sat_zen_2km(i,j) * 100
!        gk2a_ch05_2km_ref_int(i,j) = gk2a_ch05_2km_ref(i,j) * 10000
!        gk2a_ch06_2km_ref_int(i,j) = gk2a_ch06_2km_ref(i,j) * 10000
!        gk2a_ch07_2km_bt_int(i,j)  = (gk2a_ch07_2km_bt(i,j)-100)*100
!        gk2a_ch14_2km_bt_int(i,j)  = (gk2a_ch14_2km_bt(i,j)-100)*100
!        gk2a_ch15_2km_bt_int(i,j)  = (gk2a_ch15_2km_bt(i,j)-100)*100
!        ref_ano_dim_int(i,j)       = ref_ano_dim(i,j) * 100
!        ndsi_int(i,j)              = ndsi(i,j) * 10000
!        ndwi_int(i,j)              = ndwi(i,j) * 10000
!        btd_14_07_int(i,j)         = btd_14_07(i,j) * 100
!        gk2a_dem_2km_int(i,j)      = gk2a_dem_2km(i,j)
!        gk2a_lat_2km_int(i,j)      = gk2a_lat_2km(i,j) * 100
!        gk2a_lon_2km_int(i,j)      = gk2a_lon_2km(i,j) * 100
   
    ENDDO  !! X Loop END
  ENDDO  !! Y Loop END

  PRINT*, '[low confidence cloudy] :', low_conf_cld_ct
  !PRINT*, '[GK-2A cloudy] :', gk2a_cld_ct

  !!-------------------------------------------------------------------------------- 
  !! No.0 Information
  !! No.0 VIIRS Snow Cover Data
  fn = 'test.dataset_viirs.snow_'//TRIM(sc_ami_date(kk))//'.bin'
  OPEN(99,FILE=TRIM(opath_gk2a_dataset)//TRIM(fn)//'', &
          ACCESS='DIRECT', RECL=gx*gy*1)
  WRITE(99,REC=1) viirs_data_int
  CLOSE(99)

  fn = 'test.dataset_out.flag_info_'//TRIM(sc_ami_date(kk))//'.bin'
  OPEN(99,FILE=TRIM(opath_gk2a_dataset)//TRIM(fn)//'', &
          ACCESS='DIRECT', RECL=gx*gy*1)
  WRITE(99,REC=1) flag_info
  CLOSE(99)

  !! No.1 SZA
  fn = 'test.dataset_gk2a_sza_'//TRIM(sc_ami_date(kk))//'.bin'
  OPEN(99,FILE=TRIM(opath_gk2a_dataset)//TRIM(fn)//'', &
          ACCESS='DIRECT', RECL=gx*gy*4)
  WRITE(99,REC=1) gk2a_sol_zen_2km
  CLOSE(99)

  !! No.10 VZA
  fn = 'test.dataset_gk2a_vza_'//TRIM(sc_ami_date(kk))//'.bin'
  OPEN(99,FILE=TRIM(opath_gk2a_dataset)//TRIM(fn)//'', &
          ACCESS='DIRECT', RECL=gx*gy*4)
  WRITE(99,REC=1) gk2a_sat_zen_2km
  CLOSE(99)

  !! No.6 Latitude
  fn = 'test.dataset_gk2a_lat_'//TRIM(sc_ami_date(kk))//'.bin'
  OPEN(99,FILE=TRIM(opath_gk2a_dataset)//TRIM(fn)//'', &
          ACCESS='DIRECT', RECL=gx*gy*4)
  WRITE(99,REC=1) gk2a_lat_2km
  CLOSE(99)

  !! No.0 Information (Index X)
  fn = 'test.dataset_index_x_'//TRIM(sc_ami_date(kk))//'.bin'
  OPEN(99,FILE=TRIM(opath_gk2a_dataset)//TRIM(fn)//'', &
          ACCESS='DIRECT', RECL=gx*gy*2)
  WRITE(99,REC=1) gk2a_ind_x_int
  CLOSE(99)

  !! No.0 Information (Index Y)
  fn = 'test.dataset_index_y_'//TRIM(sc_ami_date(kk))//'.bin'
  OPEN(99,FILE=TRIM(opath_gk2a_dataset)//TRIM(fn)//'', &
          ACCESS='DIRECT', RECL=gx*gy*2)
  WRITE(99,REC=1) gk2a_ind_y_int
  CLOSE(99)


!-----------------------------------------------------------------------
  !! No.1 R0.46
!  fn = 'test.dataset_gk2a_ch01_ref_'//TRIM(sc_ami_date(kk))//'.bin'
!  OPEN(99,FILE=TRIM(opath_gk2a_dataset)//TRIM(fn)//'', &
!          ACCESS='DIRECT', RECL=gx*gy*4)
!  WRITE(99,REC=1) gk2a_ch01_2km_ref
!  CLOSE(99)

  !! No.2 R0.56
!  fn = 'test.dataset_gk2a_ch02_ref_'//TRIM(sc_ami_date(kk))//'.bin'
!  OPEN(99,FILE=TRIM(opath_gk2a_dataset)//TRIM(fn)//'', &
!          ACCESS='DIRECT', RECL=gx*gy*4)
!  WRITE(99,REC=1) gk2a_ch02_2km_ref
!  CLOSE(99)

  !! No.3 R0.64
!  fn = 'test.dataset_gk2a_ch03_ref_'//TRIM(sc_ami_date(kk))//'.bin'
!  OPEN(99,FILE=TRIM(opath_gk2a_dataset)//TRIM(fn)//'', &
!          ACCESS='DIRECT', RECL=gx*gy*4)
!  WRITE(99,REC=1) gk2a_ch03_2km_ref
!  CLOSE(99)

  !! No.4 R0.86
!  fn = 'test.dataset_gk2a_ch04_ref_'//TRIM(sc_ami_date(kk))//'.bin'
!  OPEN(99,FILE=TRIM(opath_gk2a_dataset)//TRIM(fn)//'', &
!          ACCESS='DIRECT', RECL=gx*gy*4)
!  WRITE(99,REC=1) gk2a_ch04_2km_ref
!  CLOSE(99)

  !! No.5 R1.38
  fn = 'test.dataset_gk2a_ch05_ref_'//TRIM(sc_ami_date(kk))//'.bin'
  OPEN(99,FILE=TRIM(opath_gk2a_dataset)//TRIM(fn)//'', &
          ACCESS='DIRECT', RECL=gx*gy*4)
  WRITE(99,REC=1) gk2a_ch05_2km_ref
  CLOSE(99)

  !! No.6 R1.61
  fn = 'test.dataset_gk2a_ch06_ref_'//TRIM(sc_ami_date(kk))//'.bin'
  OPEN(99,FILE=TRIM(opath_gk2a_dataset)//TRIM(fn)//'', &
          ACCESS='DIRECT', RECL=gx*gy*4)
  WRITE(99,REC=1) gk2a_ch06_2km_ref
  CLOSE(99) 

!-----------------------------------------------------------------------
  !! No.4 Anomaly R1.61
  fn = 'test.dataset_gk2a_ch06_ano_'//TRIM(sc_ami_date(kk))//'.bin'
  OPEN(99,FILE=TRIM(opath_gk2a_dataset)//TRIM(fn)//'', &
          ACCESS='DIRECT', RECL=gx*gy*4)
  WRITE(99,REC=1) ref_ano_dim
  CLOSE(99) 

  !! No.5 NDSI
  fn = 'test.dataset_gk2a_ndsi_'//TRIM(sc_ami_date(kk))//'.bin'
  OPEN(99,FILE=TRIM(opath_gk2a_dataset)//TRIM(fn)//'', &
          ACCESS='DIRECT', RECL=gx*gy*4)
  WRITE(99,REC=1) ndsi
  CLOSE(99) 
  ...
  !! No.5 NDWI
  fn = 'test.dataset_gk2a_ndwi_'//TRIM(sc_ami_date(kk))//'.bin'
  OPEN(99,FILE=TRIM(opath_gk2a_dataset)//TRIM(fn)//'', &
          ACCESS='DIRECT', RECL=gx*gy*4)
  WRITE(99,REC=1) ndwi
  CLOSE(99)
  ...
  !! No.5 NDVI
!  fn = 'test.dataset_gk2a_ndvi_'//TRIM(sc_ami_date(kk))//'.bin'
!  OPEN(99,FILE=TRIM(opath_gk2a_dataset)//TRIM(fn)//'', &
!          ACCESS='DIRECT', RECL=gx*gy*4)
!  WRITE(99,REC=1) ndvi
!  CLOSE(99)

  !! No.2 BTD[BT11.2-BT3.8]
  fn = 'test.dataset_gk2a_btd_11.2_3.8_'//TRIM(sc_ami_date(kk))//'.bin'
  OPEN(99,FILE=TRIM(opath_gk2a_dataset)//TRIM(fn)//'', &
          ACCESS='DIRECT', RECL=gx*gy*4)
  WRITE(99,REC=1) btd_14_07
  CLOSE(99)
!-----------------------------------------------------------------------
  !! No.7 BT3.8
  fn = 'test.dataset_gk2a_ch07_bt_'//TRIM(sc_ami_date(kk))//'.bin'
  OPEN(99,FILE=TRIM(opath_gk2a_dataset)//TRIM(fn)//'', &
          ACCESS='DIRECT', RECL=gx*gy*4)
  WRITE(99,REC=1) gk2a_ch07_2km_bt
  CLOSE(99) 

  !! No.11 BT12.3
  fn = 'test.dataset_gk2a_ch15_bt_'//TRIM(sc_ami_date(kk))//'.bin'
  OPEN(99,FILE=TRIM(opath_gk2a_dataset)//TRIM(fn)//'', &
          ACCESS='DIRECT', RECL=gx*gy*4)
  WRITE(99,REC=1) gk2a_ch15_2km_bt
  CLOSE(99) 

  !! No.12 BT11.2
  fn = 'test.dataset_gk2a_ch14_bt_'//TRIM(sc_ami_date(kk))//'.bin'
  OPEN(99,FILE=TRIM(opath_gk2a_dataset)//TRIM(fn)//'', &
          ACCESS='DIRECT', RECL=gx*gy*4)
  WRITE(99,REC=1) gk2a_ch14_2km_bt
  CLOSE(99) 
!!-----------------------------------------------------------------------
