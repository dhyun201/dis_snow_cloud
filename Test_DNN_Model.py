# -*- coding: utf-8 -*-
"""
Created on Wed Sep 29 17:53:25 2021
...
ipath_model    = 'D:/PhD/00_VERSION_3/6__DL_Make.Model/Model/'
opath          = 'D:/PhD/00_VERSION_3/7__DL_Output_Composite_NMSC.Snow/1_Target_gk2a_cld_all.scene/'
...
# -----------------------------------------------------------------------------
# File Exist Check
  f_exist = os.path.exists(ipath+"test.dataset_gk2a_ch06_ref_"+date_ami+".bin")
  if not (f_exist):
...
# Seize the Input data for data-frame
  ...
  df_t['ind_x']         = pd.Series(index=ix, data=t_ind_x,    dtype='int16') 
...
# Extract Low confidence cloudy to discriminate snow/cloud pixel
  ...
  # df_t_lcld_tmp -> Low confidence cloudy 영역에 대한 Dataset 구성
...
# Check data frame
  ...
  # df_t_lcld -> 'flag_info', 'ind_x', 'ind_y', 'viirs_data' 데이터프레임 인자 빠짐  
# -----------------------------------------------------------------------------
# -----------------------------------------------------------------------------
  out_df_t = df_t_lcld_tmp  # out_flag가 6인 pixel 정보만 간추리기 (out_flag=6 -> low confidence cloudy)
#------------------------------------------------------------------------------
# Normzalization or Standard
  ...
  # scaler = MinMaxScaler()
  # scaler.fit(df_t_lcld)
  # df_t_scaled = scaler.transform(df_t_lcld) # Test하고자 하는 Scene에 대해 MinMaxScaler 적용
  # del df_t_lcld
#------------------------------------------------------------------------------
# DNN 모델 불러오기
  model = load_model(ipath_model+"Model_v3_Layer.4_Node.500_Dropout.0.0_Regularization.L1.0_L2.0_Early.Stopping.Patience.20_Learning.Rate.0.0001_Epoch.400_Batch.Size.1024.h5")
  print('[Calling DNN model] Complete')

# 라벨데이터 원핫인코딩 처리
#  df_t_scaled_tmp = np_utils.to_categorical(df_t_scaled)


# DNN 모델 활용한 적설/구름 구분
  predicted = model.predict(df_t_scaled)
  print('[Execute the DNN model] Complete')
#------------------------------------------------------------------------------  
  thres_val = 0.5
  snow_ind = predicted >= thres_val  ;  predicted[snow_ind] = 1.0
  cld_ind = predicted <  thres_val  ;  predicted[cld_ind] = 0.0
#------------------------------------------------------------------------------  
...
# Calc. Accuracy using VIIRS Snow Cover Data
  df_t_lcld_ind_1 = (total_predict['viirs_data'] == 1) | (total_predict['viirs_data'] == 0) 
...
# Save the file
  f = open(opath+"output_DNN.model_v3_"+date_ami+".bin", "wb")
